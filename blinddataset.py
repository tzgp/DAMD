...
class Dataset_train(Data.Dataset):
    def __init__(self, opt):
        ...
        self.img = self.rgb_transform(img)
...
def BS3LNetData(opt):
    ...
    band = image.shape[2]#band = image.shape[2] 204
...
def get_mirror_hsi(height, width, band, image, patch):
    ...
    print('-' * 50)
    print("The patch size is : [{0},{1}]".format(patch,patch))#[19,19]
    print("The mirror_data size : [{0},{1},{2}]".format(mirror_hsi.shape[0],mirror_hsi.shape[1],mirror_hsi.shape[2]))#[138,138,204]
    print('-' * 50)
    return mirror_hsi
