...
class FeatureFusionBlock(nn.Module):
    ...
    def feature_fusion(self, xyz_feature, rgb_feature):
        print("feature = torch.cat([xyz_feature, rgb_feature], dim=2)", xyz_feature.shape, rgb_feature.shape)
        xyz_feature  = self.xyz_mlp(self.xyz_norm(xyz_feature.float()))
        rgb_feature  = self.rgb_mlp(self.rgb_norm(rgb_feature))

        feature = torch.cat([xyz_feature, rgb_feature], dim=2)# torch.Size([1, 3136, 1152]) torch.Size([1, 3136, 768]) torch.Size([1, 3136, 1920])
